package com.masterkenth.drops;

import com.masterkenth.ItemData;
import java.util.ArrayList;

import java.util.List;

public class Enragedbarbarianspirit implements BaseNPC {
    public List<ItemData> drops;

    public Enragedbarbarianspirit() {
        drops = new ArrayList<>();
        drops.add(new ItemData(1623, 0.00390625f, "1", false));
        drops.add(new ItemData(1621, 0.001953125f, "1", false));
        drops.add(new ItemData(1619, 0.0009765625f, "1", false));
        drops.add(new ItemData(1452, 0.00036618232950550735f, "1", false));
        drops.add(new ItemData(1462, 0.00036618232950550735f, "1", false));
        drops.add(new ItemData(1617, 0.000244140625f, "1", false));
        drops.add(new ItemData(830, 0.0001220703125f, "5", false));
        drops.add(new ItemData(987, 0.0001220703125f, "1", false));
        drops.add(new ItemData(985, 0.0001220703125f, "1", false));
        drops.add(new ItemData(1247, 0.00000762939453125f, "1", false));
        drops.add(new ItemData(2366, 0.000003814697265625f, "1", false));
        drops.add(new ItemData(1249, 0.000002860799449261776f, "1", false));
        drops.add(new ItemData(1191, 0.0625f, "1", false));
        drops.add(new ItemData(1157, 0.0078125f, "1", false));
        drops.add(new ItemData(1193, 0.0078125f, "1", false));
        drops.add(new ItemData(1143, 0.0078125f, "1", false));
        drops.add(new ItemData(1181, 0.0078125f, "1", false));
        drops.add(new ItemData(1371, 0.0078125f, "1", false));
        drops.add(new ItemData(1333, 0.0078125f, "1", false));
        drops.add(new ItemData(1391, 0.0078125f, "1", false));
        drops.add(new ItemData(560, 0.0390625f, "5", false));
        drops.add(new ItemData(565, 0.0234375f, "3", false));
        drops.add(new ItemData(886, 0.0234375f, "20", false));
        drops.add(new ItemData(1081, 0.0390625f, "1", false));
        drops.add(new ItemData(9145, 0.0234375f, "5", false));
        drops.add(new ItemData(890, 0.015625f, "5", false));
        drops.add(new ItemData(199, 0.009765625f, "1", false));
        drops.add(new ItemData(201, 0.007326007326007326f, "1", false));
        drops.add(new ItemData(203, 0.005494505494505495f, "1", false));
        drops.add(new ItemData(205, 0.004271678769756515f, "1", false));
        drops.add(new ItemData(207, 0.003356831151393085f, "1", false));
        drops.add(new ItemData(209, 0.00244140625f, "1", false));
        drops.add(new ItemData(211, 0.0018311664530305805f, "1", false));
        drops.add(new ItemData(213, 0.0015257857796765334f, "1", false));
        drops.add(new ItemData(11371, 0.0390625f, "1", false));
        drops.add(new ItemData(215, 0.001220703125f, "1", false));
        drops.add(new ItemData(2485, 0.0009154994049253868f, "1", false));
        drops.add(new ItemData(217, 0.0009154994049253868f, "1", false));
        drops.add(new ItemData(995, 0.078125f, "142", false));
        drops.add(new ItemData(995, 0.0390625f, "106", false));
        drops.add(new ItemData(995, 0.015625f, "106", false));
        drops.add(new ItemData(995, 0.0078125f, "323", false));
        drops.add(new ItemData(1515, 0.09375f, "1", false));
        drops.add(new ItemData(11341, 0.0390625f, "1", false));
        drops.add(new ItemData(11443, 0.0390625f, "1", false));
        drops.add(new ItemData(1295, 0.0390625f, "1", false));
        drops.add(new ItemData(2353, 0.0390625f, "3", false));
        drops.add(new ItemData(2355, 0.0390625f, "3", false));
        drops.add(new ItemData(2357, 0.0390625f, "1", false));
        drops.add(new ItemData(9419, 0.0234375f, "2", false));
        drops.add(new ItemData(2361, 0.015625f, "1", false));
        drops.add(new ItemData(444, 0.015625f, "1", false));
        drops.add(new ItemData(1637, 0.015625f, "1", false));
        drops.add(new ItemData(857, 0.0234375f, "1", false));
        drops.add(new ItemData(2801, 0.0078125f, "1", false));
        drops.add(new ItemData(865, 0.0234375f, "10", false));
        drops.add(new ItemData(1424, 0.015625f, "1", false));
        drops.add(new ItemData(1325, 0.015625f, "1", false));
        drops.add(new ItemData(808, 0.0078125f, "10", false));
    }

    public List<ItemData> getDrops() {
        return drops;
    }
}
